        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="sm:flex sm:items-center">
                ...
                <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <button type="button" @click="$inertia.visit(route('admin.quizzes.index'))"
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <select id="subcourse_id" v-model="form.subcourse_id"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                required>
                                <option value="">Choose a sub-course</option>
                                <option v-for="course in subCourses" :key="course.id" :value="course.id">
                                    {{ course.full_title }}
                                </option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="description" v-model="form.description" rows="3"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="Enter quiz description (optional)"></textarea>
                    ...
                    <div v-else class="space-y-6">
                        <div v-for="(question, questionIndex) in form.questions" :key="questionIndex"
                            ...
                            <div class="mb-4">
                                ...
                                <textarea :id="'question-' + questionIndex" v-model="question.question" rows="3"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                    placeholder="Enter your question" required></textarea>
                                <p v-if="form.errors[`questions.${questionIndex}.question`]"
                            ...
                            <div class="mb-4">
                                ...
                                <div class="space-y-2">
                                    <div v-for="(choice, choiceIndex) in question.choices" :key="choiceIndex"
                                        ...
                                        <input type="radio" :name="'correct-' + questionIndex" :value="choiceIndex"
                                            v-model="question.correct_choice"
                                            class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300" />
                                        <input type="text" v-model="choice.choice"
                                            class="flex-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                            placeholder="Enter choice text" required />
                                        <button v-if="question.choices.length > 2" type="button"
                                ...
                                <p v-if="form.errors[`questions.${questionIndex}.choices`]"
                                    class="mt-1 text-sm text-red-600">
                                    {{ form.errors[`questions.${questionIndex}.choices`] }}
                                </p>
                                <p v-if="form.errors[`questions.${questionIndex}.correct_choice`]"
                                    class="mt-1 text-sm text-red-600">
...
const removeChoice = (questionIndex, choiceIndex) => {
    ...
    if (question.correct_choice >= question.choices.length) {
        // If correct_choice is now out of bounds, set it to the last choice
        question.correct_choice = question.choices.length - 1;
    } else if (question.correct_choice >= choiceIndex) {
        // If we removed a choice before or at the correct_choice, decrement it
        question.correct_choice = Math.max(0, question.correct_choice - 1);
    }
};
