...
class SubscriptionController extends Controller
{
	...
	public function cancel(Request $request)
	{
		...
		// Update user status to cancelled but keep expiry to allow access until then
		$user->update([
			'subscription_status' => 'cancelled',
			// Keep subscription_expires_at unchanged so access continues until expiry
		]);

		// Also cancel the current active subscription record and disable auto-renew
		...
		if ($activeSubscription) {
			$activeSubscription->cancel('User requested cancellation');
		} else {
			// As a fallback, ensure any unexpired subscription has auto-renew disabled
			$fallback = $user->subscriptions()
				->where('expires_at', '>', now())
				->latest()
				->first();
			if ($fallback && $fallback->auto_renew) {
				$fallback->update([
					'auto_renew' => false,
					'cancelled_at' => now(),
				]);
		...
		return redirect()->route('subscription.plans')
			->with('success', 'Subscription cancelled. You will retain access until your current period expires.');
	}
	...
	public function reactivate(Request $request)
	{
		...
		// If subscription hasn't expired yet, just reactivate user access
